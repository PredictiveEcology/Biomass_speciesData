...
If the user wishes to use another data source, they will need to proving an accompanying function prefixed `prepSpeciesLayers_*`, where * corresponds to theis new data source's character string in `types`.
The user should follow the `LandR::prepSpeciesLayers_KNN` and `LandR::prepSpeciesLayers_CASFRI` functions as templates to build theirs.
