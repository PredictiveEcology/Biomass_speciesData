...
```{r setup-Biomass-speciesData, include=FALSE, eval = TRUE, cache = FALSE}
...
knitr::kable(sppEquiv2, 
             caption = "Example of species merging for simulation. Here the user wants to model (ref:Abie-bal), (ref:Abie-las) and (ref:Pinu-con) as separate species, but all (ref:Pice-sp) as a genus-level group. For this, all six species are separately identified in the \\'KNN\\' column, so that their (ref:percent) cover layers can be obtained, but in the \\'Boreal\\' column (which defines the naming convention used in the simulation in this example) all (ref:Pice-sp) have the same name. (ref:Biomass-speciesData) will merge their (ref:percent) cover data into a single layer by summing their cover per pixel.") %>%
...
-   `coverThresh` -- integer. Defines a minimum (ref:percent) cover value (from
    ...
    species layers (`speciesLayers`). Note that this will affect what species have data for an
    eventual simulation and the user will need to adjust simulation parameters accordingly
