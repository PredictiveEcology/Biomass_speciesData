...
This module downloads and pre-process species (ref:percent) cover data layers to be passed
...
-   Module "initiation" (`init` event), during which all species (ref:percent) cover layers
...
This module accesses and processes species percent cover ((ref:percent) cover) data for the
...
sources are used. It's primary output is a `RasterStack` of species (ref:percent) cover,
...
When multiple species of a given data source are to be grouped, (ref:percent) cover is summed across species of the same group within each pixel. Please see the `sppEquiv` input in <!-- section --> for information on how species groups are defined.

The module can also exclude species (ref:percent) cover layers if they don't have a minimum
(ref:percent) cover value in at least one pixel. This means that the user should still
inspect in how many pixels the species is deemed present, as it is possible that
some data have only a few pixels with high (ref:percent) cover for a given species. In this
...
-   `sppEquiv` -- a table of correspondences between different species naming
    ...
    *Picea spp.* that includes, *Picea glauca*, *Picea mariana* and *Picea    engelmannii*, they will need to provide these three species names in the
...
-   `coverThresh` -- integer. Defines a minimum (ref:percent) cover value (from 0-100) that
...
-   `types` -- character. Which (ref:percent) cover data sources are to be used (see
...
`coverThresh` , which is lowered to 5(ref:percent). The species layers (the major output of
